...
OPENAI_API_KEY = "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"
...
CONFIG = {
    ...
    "llm_model": "gpt-3.5-turbo"  # You can change this to gpt-4 or other available models
}
...
def detect_intent(user_query: str, context: Dict) -> Tuple[str, str]:
    prompt = (
        ...
        "Return the response STRICTLY in this format and nothing else: Intent: <intent>, Sub-intent: <sub-intent>\n"
        "Examples:\n"
        ...
        "'Can you also give me health status' -> Intent: CI Health Check, Sub-intent: None"
    )
    try:
        response = client.chat.completions.create(
            model=CONFIG["llm_model"],
        ...
        # More robust parsing
        intent_match = re.search(r"Intent:\s*([^,]+)(?:,\s*Sub-intent:\s*(.+))?$", intent_response)
        if intent_match:
            intent, sub_intent = intent_match.groups()
            # Validate intent against allowed values
            valid_intents = [
                "Incident Status Inquiry", "CI Health Check", "Dependency Impact Analysis",
                "Recommendations Search", "Automation Execution", "List Open Incidents with CI Health",
                "General Queries"
            ]
            intent = intent.strip()
            if intent in valid_intents:
                return intent, sub_intent.strip() if sub_intent else "None"
            else:
                print(f"Invalid intent detected: {intent}")
                return "General Queries", "None"
        else:
            print(f"Failed to parse intent response: {intent_response}")
            return "General Queries", "None"
